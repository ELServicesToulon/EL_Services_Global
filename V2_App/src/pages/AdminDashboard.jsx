...
import { LayoutDashboard, LogOut, CheckCircle, Clock, Truck, Package, XCircle, Map, User, FileText, Settings } from 'lucide-react'
            ...
            <aside className="w-20 lg:w-64 bg-slate-900 text-white flex flex-col fixed h-full transition-all duration-300 z-20">
                ...
                <nav className="flex-1 px-4 py-6 space-y-2">
                    <button 
                        ...
                        className={`flex items-center w-full px-4 py-3 rounded-xl transition-colors ${activeTab === 'bookings' && filter === 'all' ? 'bg-red-600' : 'hover:bg-slate-800'}`}
                    >
                        <LayoutDashboard className="w-5 h-5 lg:mr-3" />
                        <span className="hidden lg:inline">Vue d'ensemble</span>
                    </button>
                    <button 
                        onClick={() => { setActiveTab('bookings'); setFilter('pending'); }} 
                        className={`flex items-center w-full px-4 py-3 rounded-xl transition-colors ${activeTab === 'bookings' && filter === 'pending' ? 'bg-slate-800 text-yellow-400' : 'hover:bg-slate-800'}`}
                    >
                    ...
                    <button 
                        ...
                        className={`flex items-center w-full px-4 py-3 rounded-xl transition-colors ${activeTab === 'billing' ? 'bg-purple-600' : 'hover:bg-slate-800'}`}
                    >
                        <FileText className="w-5 h-5 lg:mr-3" />
                    ...
                    <>
                        <header className="flex justify-between items-center mb-8">
                            <div>
                                ...
                                <p className="text-gray-500">Gérez vos factures et la numérotation.</p>
                            </div>
                            <button 
                                onClick={() => setActiveTab('bookings')}
                                className="bg-white px-4 py-2 rounded-lg shadow-sm border border-gray-200 text-sm font-medium hover:bg-gray-50 flex items-center"
                            >
                                <LayoutDashboard className="w-4 h-4 mr-2" /> Retour Tournées
                            </button>
                        </header>

                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            <InvoiceGenerator onInvoiceCreated={fetchBookings} />
                            <InvoiceSettings />
                        </div>
                    </>
                ) : (
