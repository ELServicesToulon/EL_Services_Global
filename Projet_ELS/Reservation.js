...
function reserverPanier(donneesReservation) {
  Logger.log('[reserverPanier] Début du traitement du panier.');
  const lock = LockService.getScriptLock();
  if (!lock.tryLock(30000)) {
    Logger.log('[reserverPanier] Impossible d\'obtenir le verrou.');
  ...
  try {
    ...
    Logger.log('[reserverPanier] Client reçu: ' + (client ? client.email : 'Aucun'));
    if (!client) {
    ...
    const items = Array.isArray(donneesReservation && donneesReservation.items) ? donneesReservation.items : [];
    if (!items.length) {
      return { success: false, summary: "Votre panier est vide." };
    }
    let failedItemIds = [];
    ...
    if (resultatEnregistrement.isNew) {
      Logger.log('[reserverPanier] Nouveau client détecté, tentative d\'envoi ID.');
      try {
...
function envoyerDevisParEmail(donneesDevis) {
  try {
    ...
    if (!sent) throw new Error("Échec de l'envoi de l'email.");
    return { success: true };
...
function calculerInfosTourneeBase(totalStops, returnToPharmacy, dateString, startTime) {
  ...
  const extraRetour = returnToPharmacy ? 1 : 0;
  const totalSegments = arretsSupplementaires + extraRetour;
  const duree = DUREE_BASE + (totalSegments * DUREE_ARRET_SUP);
  const km = KM_BASE + (totalSegments * KM_ARRET_SUP);
...
function obtenirReservationsPourClient(email, date) {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    var statut = hasStatut ? row[statutIndex] : "Confirmée";
    var statutTxt = String(statut || '').toLowerCase();
    var matchEmail = email ? clientEmail === email.trim().toLowerCase() : true;
    var matchStatut = !hasStatut || statutTxt.indexOf("confirm") === 0;
    var matchDate = true;
    if (date) {
