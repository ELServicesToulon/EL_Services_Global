...
function testerGestionClient() {
  ...
  let sig = '';
  try {
    // Génère un lien valide pour satisfaire assertClient si la signature est active
    const linkData = generateSignedClientLink(TEST_CLIENT.email);
    // linkData.url ressemble à "...?page=gestion&email=...&exp=...&sig=..."
    if (linkData && linkData.url) {
      const url = linkData.url;
      const expMatch = url.match(/exp=([^&]+)/);
      const sigMatch = url.match(/sig=([^&]+)/);
      if (expMatch) exp = decodeURIComponent(expMatch[1]);
      if (sigMatch) sig = decodeURIComponent(sigMatch[1]);
    }
  } catch (e) {
    Logger.log("WARNING: Génération lien signé échouée dans le test: " + e.message);
  }

  const validation = validerClientParEmail(TEST_CLIENT.email, exp, sig);
